...
class NatureQN(Linear):
    ...
    def initialize_models(self):
        ...
        pixels = 512
        conv1 = nn.Conv2d(state_shape[0],
                      numb_filters[0],
                      kernel_size=filter_sizes[0],
                      stride=strides[0],
                      padding=((strides[0] - 1) * img_height - strides[0] + filter_sizes[0]) // 2)
        conv2 = nn.Conv2d(numb_filters[0],
                      numb_filters[1],
                      kernel_size=filter_sizes[1],
                      stride=strides[1],
                      padding=((strides[1] - 1) * img_height - strides[1] + filter_sizes[1]) // 2)
        conv3 = nn.Conv2d(numb_filters[1],
                      numb_filters[2],
                      kernel_size=filter_sizes[2],
                      stride=strides[2],
                      padding=((strides[2] - 1) * img_height - strides[2] + filter_sizes[2]) // 2)
        totalConv = nn.Sequential(conv1,nn.ReLU(),conv2,nn.ReLU(),conv3,nn.ReLU())
        x = n_channels * self.config.state_history
        imLength = (np.prod(totalConv(torch.zeros(1, *[img_height, img_width, x])).size()))
        model_curr = nn.Sequential(totalConv,nn.Flatten(),nn.Linear(imLength, pixels),nn.ReLU(),nn.Linear(pixels, num_actions))
        self.target_network = copy.deepcopy(model_curr)
        self.q_network = copy.deepcopy(model_curr)
        ##############################################################
        ######################## END YOUR CODE #######################
