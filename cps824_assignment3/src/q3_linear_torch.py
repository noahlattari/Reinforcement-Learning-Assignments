...
class Linear(DQN):
    ...
    def calc_loss(self, q_values : Tensor, target_q_values : Tensor,
        ...
        ##############################################################
        ##################### YOUR CODE HERE - 3-5 lines #############
        x = torch.sum(rewards[done_mask == 0])
        y = torch.sum(torch.max(target_q_values[done_mask == 0], 1)[0])
        z = torch.sum(rewards[done_mask == 1])
        sampleTotal = x + gamma * y + z
        loss = (sampleTotal - torch.sum(F.one_hot(actions, num_actions) * q_values)) ** 2
        ##############################################################
        ######################## END YOUR CODE #######################
        return loss
